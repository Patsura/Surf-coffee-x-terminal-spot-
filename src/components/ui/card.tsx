...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn("rounded-[1.35rem] border sm:rounded-[1.75rem] border-black/5 bg-[#f7f1e8] text-neutral-900 shadow-sm", className)} {...props} />
  ),
);
...
const CardDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => <p ref={ref} className={cn("text-sm leading-5 sm:leading-6 text-neutral-600", className)} {...props} />,
);
