...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-hidden">
      ...
      <section id="overview" className="relative mx-auto grid min-h-[calc(100vh-73px)] w-full max-w-7xl items-center gap-8 px-4 py-10 sm:px-6 sm:py-16 lg:grid-cols-[1.08fr_0.92fr] lg:gap-10 lg:px-8">
        <div className="pointer-events-none absolute inset-x-4 top-8 hidden h-[34rem] rounded-[3rem] border border-white/10 bg-[radial-gradient(circle_at_18%_18%,rgba(217,185,140,0.24),transparent_24rem),radial-gradient(circle_at_82%_22%,rgba(73,112,129,0.18),transparent_22rem),linear-gradient(135deg,rgba(255,255,255,0.08),rgba(255,255,255,0.02))] shadow-[0_40px_140px_rgba(0,0,0,0.42)] lg:block" />
        ...
        <motion.div className="relative" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          ...
          <h1 className="max-w-4xl text-4xl font-semibold leading-[0.98] tracking-[-0.06em] text-[#fff8ed] sm:text-6xl lg:text-7xl">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <Button asChild size="lg"><a href="#project">Смотреть проект <ArrowRight className="h-4 w-4" /></a></Button>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.15 }} className="relative grid gap-4 sm:grid-cols-2">
          <div className="glass-panel col-span-full rounded-[2rem] p-5 sm:p-6">
          ...
          {heroCards.map((card) => (
            <Card key={card.label} className="border-[#d9b98c]/10 bg-[#f7f1e8]/95 shadow-2xl shadow-black/20">
      ...
      <Section id="project" eyebrow="Project logic" title="Почему это не обычная кофейня">
        <div className="grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
          <Card className="overflow-hidden bg-[#fff8ed]">
      ...
      <Section id="scenarios" eyebrow="Guest journey" title="Сценарии гостя">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {guestScenarios.map((scenario, index) => (
            <Card key={scenario.title} className="bg-[#fff8ed]">
              <CardHeader className="p-5">
                <span className="mb-2 flex h-10 w-10 items-center justify-center rounded-2xl bg-[#181715] text-sm font-semibold text-[#d9b98c]">0{index + 1}</span>
      ...
      <Section id="why-surf" eyebrow="Brand fit" title="Почему Surf Coffee">
        <Card className="bg-[#fff8ed]">
          ...
          <CardContent className="grid gap-3 p-5 pt-0 sm:p-7 sm:pt-0 md:grid-cols-2 lg:grid-cols-3">
            {surfFit.map(([dna, fit]) => <div key={dna} className="rounded-2xl bg-white p-4 text-sm leading-6 shadow-sm"><strong>{dna}</strong><span className="mt-1 block text-neutral-600">{fit}</span></div>)}
          </CardContent>
        </Card>
      </Section>
      ...
      <Section id="value" eyebrow="Value for stakeholders" title="Что получают ключевые стороны">
        <div className="grid gap-5 lg:grid-cols-3">
          {[
            ...
            <Card key={title as string} className="bg-[#fff8ed]">
              <CardHeader className="p-5">
                <CardTitle className="text-2xl tracking-[-0.04em]">{title as string}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 p-5 pt-0">
                {(items as string[]).map((item) => <div key={item} className="flex gap-3 rounded-2xl bg-white p-3 text-sm leading-6 shadow-sm"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-[#a57945]" />{item}</div>)}
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <Section id="market" eyebrow="Market proof" title="Кофейная культура уже есть. Свободна ниша международного travel coffee spot">
        ...
        <Card className="mt-6 bg-[#fff8ed]"><CardHeader><CardTitle>Вывод по рынку</CardTitle><CardDescription className="text-base leading-8">В Благовещенске уже сформирована кофейная культура, но свободна ниша международного travel coffee spot. Конкуренты здесь работают как market proof: они подтверждают привычку, ценовой коридор и готовность аудитории к современному кофейному продукту, но не закрывают сценарий терминала Россия — Китай.</CardDescription></CardHeader></Card>
      </Section>
      ...
      <Section id="readiness" eyebrow="Project readiness" title="Проектная готовность">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {readinessCards.map((item) => (
            <Card key={item} className="bg-[#fff8ed]">
              <CardHeader className="flex-row items-start gap-3 p-5">
                <CheckCircle2 className="mt-1 h-5 w-5 shrink-0 text-[#a57945]" />
